 ...
 * graphical method for comparing two probability distributions, by plotting
 * their quantiles against each other. In addition, Q-Q plots can be used as a
 * graphical means of estimating parameters in a location-scale family of
 * distributions.
 *
 * @author Haifeng Li
 */
public class QQPlot extends Plot {
    ...
    public void paint(Graphics g) {
        ...
        double[] lowerBound = g.getLowerBound();
        double[] lowerEnd = new double[2];
        lowerEnd[0] = Math.max(lowerBound[0], lowerBound[1]);
        lowerEnd[1] = lowerEnd[0];

        double[] upperBound = g.getUpperBound();
        double[] upperEnd = new double[2];
        upperEnd[0] = Math.min(upperBound[0], upperBound[1]);
